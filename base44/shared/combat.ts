...
async function conclude(base44:any,combat:any,character:any){
  ...
  let cursor=Number(combat.rng_cursor||0);const grantedRewards:any[]=[],awarded:any[]=[];if(state==='VICTORY'&&!combat.rewards_granted)for(const [rewardIndex,reward] of (encounter.rewards||[]).entries()){cursor+=1;if(randomAt(combat.seed,cursor)<=Math.min(1,Math.max(0,Number(reward.chance??1))))awarded.push({reward,rewardIndex});}
...
export async function handleCombatCommand(base44:any, user:any, body:any, requestId:string) {
  ...
  if (body.command === 'START_ENCOUNTER') {
    ...
    let combat = await base44.asServiceRole.entities.CombatInstance.create({ game_id: character.game_id, content_version: character.content_version, character_id: character.id, party_id:partyContext.party?.id||null, encounter_definition_id: encounter.id, state: 'CREATED', round: 1, seed: simulationSeed??hashSeed(requestId), rng_cursor: 0, rewards_granted: false, version: 1 });
