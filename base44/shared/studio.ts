...
const contentTypes = ['TagDefinition','InteractionRuleDefinition','EnvironmentalFeatureDefinition','ItemRecipeDefinition','WeatherDefinition','WorldEventDefinition','LocationDefinition','Connection','NPCDefinition','NPCInstance','NPCPlacement','DialogueGraph','QuestDefinition','AchievementDefinition','ItemDefinition','AbilityDefinition','EncounterDefinition','AttributeDefinition','SkillDefinition','FormulaDefinition','EffectDefinition','StatusDefinition','LocalizationEntry','RuleExtensionDefinition','RuntimeModuleDefinition'];
...
export async function handleStudioAuthoringCommand(base44:any,user:any,body:any){
  ...
  if(body.command==='SAVE_GAME_CONFIG'){
    ...
    const patch=Object.fromEntries(Object.entries(body.values||{}).filter(([key])=>gameFields.includes(key)));
