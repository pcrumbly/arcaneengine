...
export default function WorldEnvironment({environment}){
  ...
  return <section className="grid gap-3 rounded-lg border border-white/10 bg-runtime-surface p-4 sm:grid-cols-3"><Metric icon={Clock3} label={environment.clock.display?.calendar||'World time'} value={environment.clock.display?.full||`Minute ${environment.clock.current_minute}`}/><Metric icon={CloudSun} label={`${environment.region} · ${environment.clock.phase}`} value={weather?.name||'No weather defined'}/><Metric icon={Thermometer} label="Conditions" value={weather?.temperature===undefined?(weather?.description||'Unspecified'):`${weather.temperature}° · ${weather.description||'Current temperature'}`}/></section>;
}
