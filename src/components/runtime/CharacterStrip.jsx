...
export default function CharacterStrip({ character, location, characters, game, onSelect }) {
  ...
  return <section className="grid gap-3 border-y border-white/10 bg-runtime-surface p-4 sm:grid-cols-[1.4fr_repeat(3,1fr)]">
    <div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-md bg-runtime-accent/15 text-runtime-accent font-bold">{character.name.slice(0,2).toUpperCase()}</div><div><select aria-label={`Active ${term(game,'character','character')}`} value={character.id} onChange={(e) => onSelect(e.target.value)} className="bg-transparent font-semibold outline-none">{characters.map((item) => <option className="bg-runtime-surface" value={item.id} key={item.id}>{item.name}</option>)}</select><p className="text-xs text-slate-500">Content {character.content_version}</p></div></div>
