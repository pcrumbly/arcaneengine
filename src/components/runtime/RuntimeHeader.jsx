...
export default function RuntimeHeader({ state, game }) {
  ...
  return <header className="min-h-16 border-b border-white/10 bg-runtime-surface px-4 py-2 sm:px-6 flex items-center justify-between gap-4">
    ...
    {state?.environment?.clock&&<div className="flex max-w-[11rem] items-center gap-2 rounded-md border border-white/10 px-2 py-2 text-xs text-slate-300 sm:max-w-sm sm:px-3"><Clock3 size={14} className="text-runtime-accent"/><span className="truncate">{state.environment.clock.display?.full||`Minute ${state.environment.clock.current_minute}`}</span></div>}
