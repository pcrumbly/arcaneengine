...
export default function Inventory() {
  const [data, setData] = useState(null), [query, setQuery] = useState(''), [category, setCategory] = useState('all'), [selected, setSelected] = useState(null), [busy, setBusy] = useState(false), [error, setError] = useState(''), [hasCharacter, setHasCharacter] = useState(true);
  ...
  const filtered = useMemo(() => (data?.items || []).filter((item) => (category === 'all' || item.definition.category === category) && item.definition.name.toLowerCase().includes(query.toLowerCase())), [data, query, category]);
  ...
  const action = (command, slot) => invoke({ command, characterId: data.character.id, itemId: selected.id, itemVersion: selected.version, slot, requestId: crypto.randomUUID() });
  return <div className="p-4 sm:p-6"><p className="text-xs uppercase tracking-[.22em] text-cyan-400">Character / Assets</p><h2 className="mt-1 text-2xl font-semibold">Inventory & equipment</h2><div className="mt-5"><InventorySummary summary={data.summary}/></div>
    ...
    <div className="mt-3"><InventoryTable items={filtered} onInspect={setSelected}/></div><ItemDetailsDialog item={selected} busy={busy} onClose={() => setSelected(null)} onAction={action}/>
  </div>;
}