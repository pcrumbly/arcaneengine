...
export default function Quests() {
  ...
  const action = (command) => invoke({ command, characterId: data.character.id, questDefinitionId: available ? selected.id : undefined, questInstanceId: available ? undefined : selected.id, questVersion: selected.version, requestId: crypto.randomUUID() });
  return <div className="p-4 sm:p-6"><p className="text-xs uppercase tracking-[.22em] text-cyan-400">Character / Journal</p><h2 className="mt-1 text-2xl font-semibold">Quests & objectives</h2>{error && <div role="alert" className="mt-4 rounded-md border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-200">{error}</div>}
    <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-1 rounded-md border border-white/10 bg-[#0a1728] p-1">{['active','available','completed','all'].map((value) => <button key={value} onClick={() => setTab(value)} className={`rounded px-3 py-1.5 text-xs capitalize ${tab === value ? 'bg-cyan-400 text-slate-950' : 'text-slate-400'}`}>{value}</button>)}</div><label className="relative"><Search className="absolute left-3 top-2.5 text-slate-500" size={15}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search quests" className="rounded-md border border-white/10 bg-[#0a1728] py-2 pl-9 pr-3 text-sm"/></label></div>
