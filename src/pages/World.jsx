...
import WorldTimeActions from '@/components/runtime/WorldTimeActions';

export default function World() {
  const { state, busy, error, run } = useRuntime();
  const [world,setWorld]=useState(null),[worldError,setWorldError]=useState(''),[worldBusy,setWorldBusy]=useState(false),[pendingExitId,setPendingExitId]=useState(''),[pendingTimeAction,setPendingTimeAction]=useState('');
  ...
  const passTime=async selection=>{if(!world?.environment)return;setPendingTimeAction(selection.action);const result=await run({command:'ADVANCE_WORLD_TIME',characterId:state.character.id,characterVersion:state.character.version,clockVersion:world.environment.clock.version,...selection,requestId:crypto.randomUUID()});setPendingTimeAction('');if(result)await loadWorld(state.character.id)};
