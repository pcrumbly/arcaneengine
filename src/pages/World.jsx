...
export default function World() {
  ...
  const passTime=async action=>{if(!world?.environment)return;setPendingTimeAction(action);const result=await run({command:'ADVANCE_WORLD_TIME',characterId:state.character.id,characterVersion:state.character.version,clockVersion:world.environment.clock.version,action,requestId:crypto.randomUUID()});setPendingTimeAction('');if(result)await loadWorld(state.character.id)};
